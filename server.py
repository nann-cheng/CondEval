...
import secrets
from fss import FSS_INPUT_LEN,FSS_RING_LEN,GroupElement,ICNew, NewICKey,FlexKey
import time
import sys
import asyncio
...
class Server:
    ...
    def receiveCircuit(self,all):
        for index,key in enumerate(CIRCUIT):
            if key == "fss1":
                self.circuit[key] = [ NewICKey.unpack( all[index][i], 1 ) for i in range(FSS_AMOUNT) ]
            else:
    ...
    def onMulGate(self,keys):
        ...
        for i in range(2):
            ...
            if keys[0] == "ip_out":
                ret = mod_sub(ret, ring_mul(self.in_wire1,self.circuit[keys[0]][0][i],AUTHENTICATED_MODULO), AUTHENTICATED_MODULO)
                ret = mod_sub(ret, ring_mul(self.in_wire0,self.circuit[keys[1]][0][i],AUTHENTICATED_MODULO), AUTHENTICATED_MODULO)
            else:
                ret = mod_sub(ret, ring_mul(self.in_wire1,self.circuit[keys[0]][i],AUTHENTICATED_MODULO), AUTHENTICATED_MODULO)
                ret = mod_sub(ret, ring_mul(self.in_wire0,self.circuit[keys[1]][i],AUTHENTICATED_MODULO), AUTHENTICATED_MODULO)
    ...
    def onFinalFssReveal(self,keys,index):
        ...
        for i in range(2):
            if i==1:
                ret = ring_mul(self.in_wire0,self.circuit["alpha"],AUTHENTICATED_MODULO)
            else:
                if self.id==0:
                    ret = self.in_wire0
                else:
    ...
    def onFssCmp(self,maskVal_vec,key):
        ...
        ic = ICNew(ring_len = 1)
        if key == "fss1":
            ret.extend( [ic.eval(self.id, GroupElement(v,FSS_INPUT_LEN) , self.circuit[key][i])  for i,v in enumerate(maskVal_vec) ]  )
        elif key == "fss2":
            for i,v in enumerate(maskVal_vec):
                ret.extend( 
                    [bytes( ic.eval( self.FSS2Key[i].id, GroupElement(v,FSS_INPUT_LEN), self.FSS2Key[i].key ).packData() )]
                 )
    ...
    def innerProductWithMulOut(self,keys):
        ...
        for i in range(2):
            ...
            ret = vec_sub(ret, vec_mul(self.in_wire1,in_s,AUTHENTICATED_MODULO), AUTHENTICATED_MODULO)
            ret = vec_sub(ret, vec_mul(self.in_wire0,in_v,AUTHENTICATED_MODULO), AUTHENTICATED_MODULO)
...
async def async_main(_id):
    ...
    pool = Pool()
    pool.add_http_server(addr=SERVER_IPS[_id], port=NETWORK_SERVER_PORTS[_id])
    pool.add_http_client("server", addr=SERVER_IPS[1-_id], port=NETWORK_SERVER_PORTS[1-_id])
    ...
    for index in range(TEST_NUM):
        ...
        share=None
        with open('/home/crypto/Desktop/overleaf-CondEval/impl/data/data.pkl'+str(_id), 'rb') as file:
        ...
        ipReveals = [ ring_add(a,b,AUTHENTICATED_MODULO) for (a,b) in zip(mShares[0],otherShares[0])]
        ...
        finalReveals = [ ring_add(a,b,AUTHENTICATED_MODULO) for (a,b) in zip(mTruncShare, otherTruncShare)]
